def _parse_prop_selector(soup, selector: str):
    if prop_elem := soup.select_one(f".card .text-onyx td:-soup-contains('{selector}') + td"):
...
def parse_property_features(soup):
    if diff_cols := _parse_all_selector(soup).difference(PROPERTY_COLS.values()):
        ...
        return {colname: _parse_prop_selector(soup=soup, selector=selector) for colname, selector in PROPERTY_COLS.items()}
